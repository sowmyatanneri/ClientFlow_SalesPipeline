...
callservice() {
  ...
  #port=8080
  onesecond=1000
  #filename="https://ffs385ccr0.execute-api.us-east-2.amazonaws.com/loadCSV_dev"
  #while read -r line
  #do
   # parurl=$line
  #done < "$filename"
...
#  for (( i=1 ; i <= $totalruns; i++ ))
  ...
  do
    ...
    echo "#############################"
    echo "Invoking Query Service" 
    echo "#############################"
    json1={"\"filename\"":"\"databasepipeline.db\",\"bucketname\"":\"test.bucket.562.firsttest\","\"seqNum\"":0,"\"threadId\"":$threadid}
    time output=`aws lambda invoke --invocation-type RequestResponse --function-name Query --region us-east-1 --payload $json1 /dev/stdout | head -n 1 | head -c -2 ; echo`
...
echo "runs per thread $runsperthread"
runsperthread=${runsperthread%.*}
date
echo "Setting up test: runsperthread=$runsperthread threads=$threads"
for (( i=1 ; i <= $threads ; i ++))
do
  arpt+=($runsperthread)
done
